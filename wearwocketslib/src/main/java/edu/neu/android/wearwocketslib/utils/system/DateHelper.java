...
public class DateHelper {
	...
	public static Date getPreviousHourDate(Date currentDate){
    ...
    public static boolean isYesterday(long aSystemTime) {
        Date theDate = DateHelper.getDate(aSystemTime + 24 * 60 * 60 * 1000);
...
//        Log.e("TEST","Daynow: " + now.get(Calendar.DAY_OF_MONTH));
	...
	public static boolean isMinuteBefore(Date compareDate, Date nowDate){
		Calendar calendar = Calendar.getInstance();
		...
		if(calendar.after(now) && calendar.get(Calendar.MINUTE) == now.get(Calendar.MINUTE)){
			return true;
		}else{
			return false;
		}
	...
	public static boolean isFrom5MinTo55Min(Date nowDate){
	...
	public static boolean isLastHour(Date aDate){
		Calendar aCal = Calendar.getInstance();
		aCal.setTime(aDate);
		aCal.add(Calendar.HOUR_OF_DAY, 1);
		return !isHourBefore(aCal.getTime());
	}
