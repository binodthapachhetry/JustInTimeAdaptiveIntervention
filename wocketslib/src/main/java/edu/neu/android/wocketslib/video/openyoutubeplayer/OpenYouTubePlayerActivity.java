...
public class OpenYouTubePlayerActivity extends BaseActivity {
	...
	public void onCreate(Bundle pSavedInstanceState) {
		...
		if (lVideoSchemeStr != null) {
			if (lVideoSchemeStr.equalsIgnoreCase(SCHEME_YOUTUBE_PLAYLIST)){
				lYouTubeId = new PlaylistId(lVideoIdStr);
			} else if (lVideoSchemeStr.equalsIgnoreCase(SCHEME_YOUTUBE_VIDEO)) {
			...
			try {
				...
				if ((lWifiManager.isWifiEnabled() && lWifiManager.getConnectionInfo() != null && lWifiManager.getConnectionInfo().getIpAddress() != 0) ||
					(type == TelephonyManager.NETWORK_TYPE_UMTS || type == TelephonyManager.NETWORK_TYPE_EVDO_0 ||
					 ...
					 type == TelephonyManager.NETWORK_TYPE_HSUPA || type == TelephonyManager.NETWORK_TYPE_HSPA ||
					 type == 12/*NETWORK_TYPE_EVDO_B*/ || type == 14/*NETWORK_TYPE_EHRPD*/ ||
					 type == 15/*NETWORK_TYPE_HSPAP*/ || type == 13/*NETWORK_TYPE_LTE*/) 					  
